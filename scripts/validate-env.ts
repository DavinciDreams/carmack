...
import { getEnvironmentConfig, validateLLMConfig, getLLMConfig } from '../src/config/environment.js';
...
function addResult(category: string, status: 'pass' | 'warn' | 'fail', message: string, suggestion?: string) {
...
async function validateEnvironment() {
  ...
  try {
    ...
    if (['development', 'staging', 'production'].includes(env.NODE_ENV)) {
      ...
      addResult('Core', 'fail', `Invalid NODE_ENV: ${env.NODE_ENV}`, 'Set NODE_ENV to development, staging, or production');
    }
    ...
    if (env.CARMACK_REPOSITORY_URL) {
      ...
      try {
        ...
        addResult('Repository', 'fail', 'Repository URL is invalid', 'Provide a valid HTTPS or SSH Git URL');
      }
    } else if (env.NODE_ENV === 'production') {
      addResult('Repository', 'fail', 'Repository URL required in production', 'Set CARMACK_REPOSITORY_URL');
    } else {
      addResult('Repository', 'warn', 'Repository URL not configured', 'Set CARMACK_REPOSITORY_URL for Git integration');
    }
    ...
    if (existsSync(env.CARMACK_WORKSPACE)) {
      ...
      addResult('Repository', 'warn', `Workspace directory does not exist: ${env.CARMACK_WORKSPACE}`, 'Directory will be created automatically');
    }
    ...
    switch (llmConfig.provider) {
      case 'openai':
        if (llmConfig.apiKey) {
          ...
          addResult('LLM', 'warn', 'OpenAI API key not configured', 'Set OPENAI_API_KEY for OpenAI integration');
        }
        break;
      ...
      case 'anthropic':
        if (llmConfig.apiKey) {
          ...
          addResult('LLM', 'fail', 'Anthropic API key required in production', 'Set ANTHROPIC_API_KEY');
        } else {
          addResult('LLM', 'warn', 'Anthropic API key not configured', 'Set ANTHROPIC_API_KEY for Anthropic integration');
        }
        break;
      ...
      case 'mock':
        if (env.NODE_ENV === 'production') {
          addResult('LLM', 'warn', 'Using mock LLM provider in production', 'Consider using a real LLM provider');
        } else {
    ...
    if (env.ENABLE_DAFNY_VERIFICATION) {
      if (existsSync(env.DAFNY_PATH)) {
        ...
        addResult('Quality', 'warn', 'Dafny not found', 'Install Dafny or set DAFNY_PATH, or disable with ENABLE_DAFNY_VERIFICATION=false');
      }
    } else {
      addResult('Quality', 'warn', 'Dafny verification disabled', 'Enable with ENABLE_DAFNY_VERIFICATION=true for formal verification');
    }
    ...
    if (extensions.every(ext => ext.startsWith('.'))) {
      ...
      addResult('Transformation', 'fail', 'Invalid file extensions format', 'File extensions must start with a dot (e.g., .ts,.js)');
    }
    ...
    if (env.MAX_TRANSFORMATION_TIME >= 30000) {
      ...
      addResult('Performance', 'warn', 'Transformation timeout is very low', 'Consider increasing MAX_TRANSFORMATION_TIME');
    }
    
    if (env.MAX_MEMORY_USAGE >= 512) {
      addResult('Performance', 'pass', `Memory limit: ${env.MAX_MEMORY_USAGE}MB`);
    } else {
      addResult('Performance', 'warn', 'Memory limit is very low', 'Consider increasing MAX_MEMORY_USAGE');
    }
    ...
    if (env.CARMACK_TELEMETRY_ENABLED) {
      ...
      if (env.METRICS_ENDPOINT) {
        ...
        addResult('Telemetry', 'warn', 'No metrics endpoint configured', 'Set METRICS_ENDPOINT for external metrics collection');
      }
    } else {
      addResult('Telemetry', 'warn', 'Telemetry disabled', 'Enable with CARMACK_TELEMETRY_ENABLED=true for observability');
    }
    ...
    if (env.ENABLE_BACKUPS) {
      ...
      if (existsSync(backupDir)) {
        ...
        addResult('Backup', 'warn', `Backup directory will be created: ${env.BACKUP_DIRECTORY}`, 'Directory will be created automatically');
      }
    } else {
    ...
    if (env.ENABLE_FILE_LOGGING) {
      if (existsSync(logDir)) {
        ...
        addResult('Logging', 'warn', 'Log directory will be created', 'Directory will be created automatically');
      }
    }
    ...
    if (env.API_SECRET_KEY) {
      ...
      addResult('Security', 'warn', 'API secret key not configured', 'Set API_SECRET_KEY for production security');
    }
    ...
    addResult('Core', 'fail', `Configuration validation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
