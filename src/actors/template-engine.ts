...
const TemplatePatternSchema = z.object({
  ...
  replacement: z.object({
    ...
    transformers: z.record(z.enum(['camelCase', 'pascalCase', 'kebabCase', 'uppercase', 'lowercase', 'trim', 'escape'])).optional(),
...
export const templateEngineActor = fromPromise(
  async ({ input }: { input: TemplateTransformationRequest }) => {
    ...
    console.log(`🚀 Starting ultra-fast template transformations on ${validatedInput.targetFiles.length} files`);
    
    const results = await applyTemplateTransformations(validatedInput);
    
    console.log(`✨ Template engine completed: ${results.transformationsApplied} transformations across ${results.filesModified.length} files`);
  ...
  for (const filePath of request.targetFiles) {
    try {
      ...
      if (transformResult.transformations.length > 0) {
        ...
        console.log(`📝 Template-transformed ${filePath}: ${transformResult.transformations.length} patterns applied`);
      }
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    for (let i = 0; i < variableNames.length; i++) {
      ...
      if (varValue !== undefined && varName !== undefined) {
        const context = extractVariableContext(varValue, match.index + match[0].indexOf(varValue), content);
        variables.push({
          name: varName,
          value: varValue,
...
function inferVariableType(value: string): TemplateVariable['type'] {
  ...
  if (/^(['"`]).*\1$/.test(trimmed) || /^\d+(\.\d+)?$/.test(trimmed) || /^(true|false|null|undefined)$/.test(trimmed)) {
    return 'literal';
  }
 ...
 */
function matchesContext(match: TemplateMatch, context?: TemplatePattern['pattern']['context']): boolean {
  ...
  if (context.notInside) {
    for (const forbidden of context.notInside) {
      switch (forbidden) {
        ...
        case 'string':
          const quotes = ['"', "'", '`'];
          if (quotes.some(q => precedingCode.lastIndexOf(q) > precedingCode.lastIndexOf(q + q))) return false;
          break;
      }
    }
  }
  ...
  if (pattern.replacement.conditionals) {
    for (const conditional of pattern.replacement.conditionals) {
      try {
        ...
        const context = match.variables.reduce((ctx, variable) => {
...
function evaluateCondition(condition: string, context: Record<string, string>): boolean {
  try {
    ...
    for (const [varName, value] of Object.entries(context)) {
      ...
      evaluableCondition = evaluableCondition.replace(new RegExp(`\\b${varName}\\b`, 'g'), escapedValue);
    }
