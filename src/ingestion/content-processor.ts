...
export const ProcessingResultSchema = z.object({
  ...
  embeddings: z.array(z.array(z.number()).length(384)),
...
export class ContentProcessor {
  ...
  constructor(config?: Partial<ContentProcessingConfig>) {
    this.config = ContentProcessingConfigSchema.parse(config || {});
    this.hfToken = getEnvironmentConfig().HF_TOKEN;
  }
  ...
  async processFileContent(
    ...
    try {
      console.log(`🔄 Processing content for: ${fileContent.relativePath}`);
  ...
  async processFiles(
    ...
    for (let i = 0; i < fileContents.length; i += this.config.batchSize) {
      ...
      const batchPromises = batch.map(async (fileContent) => {
        try {
          const astResult = astResults?.get(fileContent.relativePath);
          return await this.processFileContent(fileContent, astResult);
        } catch (error) {
          console.warn(`⚠️ Failed to process ${fileContent.relativePath}:`, error);
          return null;
        }
      });
   ...
   */
  private chunkContent(fileContent: FileContent): ContentChunk[] {
   ...
   */
  private chunkCodeContent(fileContent: FileContent): ContentChunk[] {
    const content = fileContent.content;
  ...
  private generateHeuristicAnnotation(
    ...
    const content = fileContent.content;
    const path = fileContent.relativePath;
   ...
   */
  private generateSummary(fileContent: FileContent, domains: string[]): string {
    const fileName = fileContent.relativePath.split('/').pop() || 'file';
   ...
   */
  private inferPurpose(fileContent: FileContent, domains: string[]): string {
    const path = fileContent.relativePath;
    const content = fileContent.content;
