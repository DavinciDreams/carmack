...
import type { BenchmarkResult, HistoricalBugScenario } from '../types.js';
import { validateBenchmarkResult } from '../types.js';
import { HISTORICAL_BUG_SCENARIOS } from './historical-scenarios.js';
    ...
    for (let i = 0; i < evidenceCount; i++) {
      evidence.push({
        ...
        line_range: {
          ...
          end: Math.floor(Math.random() * 50) + 150,
        },
...
export class BenchmarkEngine {
  ...
  async runBenchmarkSuite(config: BenchmarkConfig = {}): Promise<BenchmarkSuiteResult> {
    ...
    const averageSpeedImprovement =
      successfulResults.length > 0
        ? successfulResults.reduce((sum, r) => sum + r.speedImprovement, 0) /
   ...
   */
  async runScenario(
    scenario: HistoricalBugScenario,
    config: BenchmarkConfig
    ...
    try {
      ...
      const speedImprovement = this.calculateSpeedImprovement(
        executionTime,
        scenario.manualInvestigationTime
      );
  ...
  private calculateAccuracy(response: QueryResponse, scenario: HistoricalBugScenario): number {
    ...
    if (
      scenario.category === 'scheduler' &&
      response.primary_answer.toLowerCase().includes('scheduler')
    ) {
      accuracy += 0.05;
    }
    if (
      scenario.category === 'memory' &&
      response.primary_answer.toLowerCase().includes('memory')
    ) {
      accuracy += 0.05;
    }
  ...
  private calculateRelevanceScore(response: QueryResponse): number {
    const evidenceRelevance = response.evidence_chain.reduce(
      (sum, item) => sum + item.relevance_score,
      0
    );
    return response.evidence_chain.length > 0
      ? evidenceRelevance / response.evidence_chain.length
      : 0;
  }
  ...
  private generateRecommendations(results: BenchmarkResult[], targetsMet: any): string[] {
    ...
    if (!targetsMet.speedImprovement) {
      recommendations.push(
        'Optimize query processing pipeline to achieve 75% speed improvement target'
      );
    }

    if (!targetsMet.responseTime) {
      recommendations.push(
        'Reduce response time to under 2 seconds through caching and indexing improvements'
      );
    }

    if (!targetsMet.accuracy) {
      recommendations.push(
        'Improve accuracy through better evidence ranking and response synthesis'
      );
    }
    ...
    if (failedResults.length > 0) {
      recommendations.push(
        `Address ${failedResults.length} failed scenarios to improve reliability`
      );
    }
