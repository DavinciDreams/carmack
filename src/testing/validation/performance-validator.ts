...
import { z } from 'zod';
import type { LoadTestResult, PerformanceMetrics } from '../../types/unified-schemas';
import { validateLoadTestResult, validatePerformanceMetrics } from '../../types/unified-schemas';
...
export class PerformanceValidator {
  ...
  async validateConcurrentUsers(
    testQueries: unknown,
    userCount: unknown = 100,
    testDuration: unknown = 300000
    ...
    try {
      const simulators = Array.from(
        { length: users },
        (_, i) => new UserSimulator(`user_${i + 1}`, queries)
      );
  ...
  private async warmup(queries: string[]): Promise<void> {
    await Promise.all(queries.map(() => Promise.resolve()));
  }
  private async executeQuery(_query: string): Promise<void> {
  ...
  async validateDatabasePerformance(
    ...
    try {
      ...
      const summary = {
        averageResponseTime:
          queryTimes.length > 0 ? queryTimes.reduce((sum, t) => sum + t, 0) / queryTimes.length : 0,
        p95ResponseTime:
          queryTimes.length > 0 ? queryTimes[Math.floor(queryTimes.length * 0.95)] || 0 : 0,
        p99ResponseTime:
          queryTimes.length > 0 ? queryTimes[Math.floor(queryTimes.length * 0.99)] || 0 : 0,
      ...
      const passed =
        summary.averageResponseTime <= this.config.databaseQueryTimeoutMs &&
  ...
  async validateResponseTime(
    ...
    try {
      ...
      const passed =
        summary.averageResponseTime <= this.config.responseTimeTarget &&
      ...
      console.log(
        `   Average: ${summary.averageResponseTime.toFixed(0)}ms (Target: ${this.config.responseTimeTarget}ms)`
      );
      console.log(`   P95: ${summary.p95ResponseTime.toFixed(0)}ms`);
      console.log(`   Success Rate: ${(summary.successRate * 100).toFixed(1)}%`);
      console.log(`   Result: ${passed ? 'PASSED' : 'FAILED'}`);
  ...
  private generateConcurrentUserRecommendations(
    summary: any,
    userCount: number,
    passed: boolean
  ): string[] {
