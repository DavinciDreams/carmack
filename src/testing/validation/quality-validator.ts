...
type DataIntegrityInput = undefined;

export const dataIntegrityActor = fromPromise(async () => {
      ...
      violations: [
        {
          ...
          description: 'Orphaned relationships',
          severity: 'high',
          count: 150,
        },
        {
...
function calculateRelationshipAccuracy(
  _expectedPaths: string[][],
  _actualPaths: string[][]
): number {
...
export async function validateGraphTraversal(traversalTests: GraphTraversalInput): Promise<any[]> {
  ...
  async ({ input }: { input: SearchRelevanceInput }) => {
    ...
    for (const { query, results, expectedRelevance } of input) {
      ...
      const dcg = resultsWithRelevance.reduce((sum, r) => {
        const gain = 2 ** r.relevanceRating - 1;
        const discount = Math.log2(r.rank + 1);
        return sum + gain / discount;
      }, 0);
      const sortedRelevance = resultsWithRelevance
        .map((r) => r.relevanceRating)
        .sort((a, b) => b - a);
      ...
      // Precision, Recall, F1
      const precision =
        resultsWithRelevance.length > 0 ? relevantResults.length / resultsWithRelevance.length : 0;
      const recall =
        expectedRelevance && expectedRelevance.length > 0
          ? relevantResults.length / expectedRelevance.length
          : 0;
...
export async function evaluateSearchRelevance(searchResults: SearchRelevanceInput): Promise<any[]> {
...
import { type AccuracyValidation, AccuracyValidationSchema } from '../types.ts';
...
export const QualityValidationConfigSchema = z
  .object({
...
class MockKnowledgeBase {
  ...
  private initializeGroundTruth(): void {
    this.groundTruthData.set('scheduler_preemption', {
      expectedAnswer:
        'TensorRT-LLM scheduler uses priority-based preemption with request queuing and dynamic batching optimization',
    ...
    this.groundTruthData.set('memory_management', {
      expectedAnswer:
        'TensorRT-LLM implements memory pooling, garbage collection, and dynamic allocation strategies optimized for GPU memory constraints',
    ...
    this.groundTruthData.set('cuda_kernels', {
      expectedAnswer:
        'CUDA kernels in TensorRT-LLM include fused operations, memory coalescing improvements, and architecture-specific optimizations',
      keyPoints: [
        'fused operations',
        'memory coalescing',
...
function calculateRelevanceScore(response: QueryResponse, query: string): number {
  ...
  const evidenceRelevance =
    response.evidence_chain.length > 0
      ? response.evidence_chain.reduce((sum, e) => sum + e.relevance_score, 0) /
        response.evidence_chain.length
      : 0;
