...
export const patternRoutes = [
  {
    ...
    handler: async (req: Request) => {
      ...
      if (!idParse.success) {
        return new Response(JSON.stringify({ error: 'Invalid pattern id', details: idParse.error.errors }), { status: 400 });
      }
      ...
      if (!parsed.success) {
        return new Response(JSON.stringify({ error: 'Invalid pattern metadata', details: parsed.error.errors }), { status: 400 });
      }
