...
export const fileRoutes = [
  {
    ...
    handler: async (req: Request) => {
      ...
      if (!idParse.success) {
        return new Response(JSON.stringify({ error: 'Invalid file id', details: idParse.error.errors }), { status: 400 });
      }
      ...
      if (!parsed.success) {
        return new Response(JSON.stringify({ error: 'Invalid file metadata', details: parsed.error.errors }), { status: 400 });
      }
