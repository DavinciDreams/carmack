...
const LefthookCommandSchema = z.object({
...
const LefthookHookSchema = z.object({
  parallel: z.boolean().optional(),
  commands: z.record(LefthookCommandSchema),
}).strict();

export const LefthookConfigSchema = z.object({
...
const DockerComposeServiceSchema = z.object({
...
const PrometheusScrapeConfigSchema = z.object({
  job_name: z.string(),
  static_configs: z.array(z.object({
...
export const PrometheusConfigSchema = z.object({
  ...
  alerting: z.object({
    alertmanagers: z.array(z.object({
      static_configs: z.array(z.object({
        targets: z.array(z.string()),
      })),
    })),
...
const GitHubActionStepSchema = z.object({
...
const GitHubActionJobSchema = z.object({
...
export const GitHubWorkflowSchema = z.object({
  ...
  on: z.union([
    ...
    z.object({
      ...
      workflow_dispatch: z.record(z.any()).optional(),
    })
  ]),
...
export class ConfigValidator {
   ...
   */
  static async validateLefthook(filePath: string = 'lefthook.yml'): Promise<{
    valid: boolean;
    errors?: string[];
   ...
   */
  static async validateDockerCompose(filePath: string = 'docker-compose.yml'): Promise<{
    valid: boolean;
    errors?: string[];
  ...
  static async validateAllConfigs(): Promise<{
    ...
    try {
      results.lefthook = await this.validateLefthook();
    } catch (error) {
      results.lefthook = {
        valid: false,
    ...
    try {
      results.dockerCompose = await this.validateDockerCompose();
    } catch (error) {
    ...
    try {
      results.prometheus = await this.validatePrometheus('monitoring/prometheus.yml');
    } catch (error) {
      results.prometheus = {
        valid: false,
        errors: ['File not found or not accessible']
    ...
    for (const file of workflowFiles) {
      try {
        const result = await this.validateGitHubWorkflow(file);
