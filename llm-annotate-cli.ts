...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log(
      '  --output <directory>    Output directory for annotations (default: ./output/annotations)'
    );
    console.log('  --format <json|yaml|markdown>  Output format (default: json)');
    console.log(
      '  --include <patterns>    Include file patterns (comma-separated, default: all supported files)'
    );
    process.exit(1);
  }
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (arg === '--max-files' && i + 1 < args.length) {
      maxFiles = Number.parseInt(args[i + 1]);
      i++; // Skip next arg
