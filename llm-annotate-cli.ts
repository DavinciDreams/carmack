...
import { parseArgs, showHelp, findCodeFiles, generateLLMAnnotations } from './src/llm-annotation/index.js';
 ...
 * CLI for LLM Code Annotation
 ...
 async function main() {
    ...
    // Parse CLI arguments using shared logic
    let options;
    try {
      options = parseArgs(args);
    } catch (err) {
      console.error('❌ Argument parsing error:', err instanceof Error ? err.message : err);
      showHelp();
      process.exit(1);
    }
    ...
    const maxFiles = 50; // Could be made configurable if needed
   ...
   try {
     // Find all code files in the directory using shared logic
     const sourceFiles = await findCodeFiles(
       directory,
       maxFiles,
       options.include && options.include.length > 0 ? options.include : undefined
     );
     ...
     const request: AnnotationRequest = {
       ...
       includePrompts: options['no-prompts'] ? false : true,
       includePatterns,
       excludePatterns: options.exclude || ['node_modules/**', '**/*.test.*', '.git/**', 'build/**', 'dist/**'],
