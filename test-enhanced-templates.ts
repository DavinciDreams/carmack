...
async function testTemplateTransformations() {
  ...
  try {
    ...
    // Performance metrics
    console.log('\n⚡ Performance Metrics:');
    console.log(`  🏃‍♂️ Speed: ${Math.round(result.transformationsApplied / (duration / 1000))} transformations/second`);
    console.log(`  📈 Efficiency: ${(result.transformationsApplied / testCode.length * 1000).toFixed(2)} transformations per 1K characters`);
...
async function benchmarkTemplateVsAST() {
  ...
  try {
    ...
    const astPatterns = patternsData.patterns
      .filter((p: any) => p.mode === 'ast')
      .slice(0, 5); // Take first 5 for fair comparison
      
    console.log(`🧪 Testing with ${templatePatterns.length} template patterns vs ${astPatterns.length} AST patterns`);
    ...
    console.log('├─────────────────┼──────────────┼──────────────┼─────────────┤');
    console.log(`│ Template        │ ${templateDuration.toString().padEnd(12)} │ ${templateResult.transformationsApplied.toString().padEnd(12)} │ ${Math.round(templateResult.transformationsApplied / (templateDuration / 1000)).toString().padEnd(11)} │`);
    console.log(`│ AST             │ ${astDuration.toString().padEnd(12)} │ ${astResult.transformationsApplied.toString().padEnd(12)} │ ${Math.round(astResult.transformationsApplied / (astDuration / 1000)).toString().padEnd(11)} │`);
    console.log('└─────────────────┴──────────────┴──────────────┴─────────────┘');
