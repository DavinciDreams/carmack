...
export class BamlAsyncClient {
  ...
  async ExtractSemanticAnnotation(
    content: string,
    __baml_options__?: BamlCallOptions
  ): Promise<types.SemanticAnnotation> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) };
      const collector = options.collector
        ? Array.isArray(options.collector)
      ...
      return raw.parsed(false) as types.SemanticAnnotation;
    } catch (error) {
      throw toBamlError(error);
    }
  }
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.SemanticAnnotation, types.SemanticAnnotation> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) };
      const collector = options.collector
        ? Array.isArray(options.collector)
      ...
      return new BamlStream<partial_types.SemanticAnnotation, types.SemanticAnnotation>(
        raw,
        (a): partial_types.SemanticAnnotation => a,
        (a): types.SemanticAnnotation => a,
        this.ctxManager.cloneContext()
      );
    } catch (error) {
      throw toBamlError(error);
