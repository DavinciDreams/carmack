...
async function runTemplateEngineTest() {
  ...
  const testFiles = ['test-ast-vs-template.ts', 'test-ast-patterns.ts'];
  ...
  try {
    ...
    console.log('\n⚡ Performance Analysis:');
    console.log(
      `  🏃‍♂️ Average time per transformation: ${(duration / result.transformationsApplied).toFixed(2)}ms`
    );
    console.log(
      `  📈 Transformations per second: ${Math.round(result.transformationsApplied / (duration / 1000))}`
    );
