...
describe('Pattern Learning System', () => {
  ...
  const testDataDir = './test-pattern-data/data';
  ...
  afterEach(async () => {
    try {
      // Only clean up test data files, not the main data directory
      await unlink(join(testDataDir, 'pattern-effectiveness.json'));
      await unlink(join(testDataDir, 'discovered-patterns.json'));
    } catch {
  ...
  describe('PatternLearner Class', () => {
    test('should create pattern learner with default configuration', () => {
      const learner = new PatternLearner(testDataDir);
    ...
    test('should persist and load learning data', async () => {
      const learner1 = new PatternLearner(testDataDir);
      ...
      const learner2 = new PatternLearner(testDataDir);
